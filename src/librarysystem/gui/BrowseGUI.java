...
public class BrowseGUI extends JPanel {
							...
							.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
								.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
									...
									.addGroup(groupLayout.createSequentialGroup()
										...
										.addContainerGap())
									.addGroup(Alignment.LEADING, groupLayout.createSequentialGroup()
										.addComponent(lblTes, GroupLayout.PREFERRED_SIZE, 343, GroupLayout.PREFERRED_SIZE)
										.addContainerGap()))
								.addGroup(groupLayout.createSequentialGroup()
				...
				.addGroup(groupLayout.createSequentialGroup()
					...
					.addGroup(groupLayout.createParallelGroup(Alignment.LEADING, false)
						...
						.addGroup(groupLayout.createSequentialGroup()
							...
							.addComponent(btnNewButton)
							.addGap(36)
							.addComponent(lblTes, GroupLayout.PREFERRED_SIZE, 134, GroupLayout.PREFERRED_SIZE)))
					.addContainerGap())
		);
	...
	public void sort(String stringSort, MaterialType materialType) {
		...
		if (materialType == MaterialType.ALL) {
			materials = librarySystem.getMaterialManager().getUniqueMaterials(MaterialStatus.AVAILABLE);
		} else {
			materials = librarySystem.getMaterialManager().getUniqueMaterials(MaterialStatus.AVAILABLE, materialType);
		}
		
		if (stringSort.equalsIgnoreCase("Title")) {
			materials.sort(new TitleComparator());
		} else if (stringSort.equalsIgnoreCase("Author")) {
			materials.sort(new AuthorComparator());
		}
		
		DefaultTableModel model = (DefaultTableModel) table.getModel();
		for (int i = model.getRowCount() - 1; i >= 0; i--) {model.removeRow(i);}
		for (int i = 0; i < materials.size(); i++) {
			model.addRow(new Object[]{null, materials.get(i).getNiceName()});
		}
	}
}
